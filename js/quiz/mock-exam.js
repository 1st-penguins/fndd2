...
// Q+W 동시입력 테스트용 (관리자 전용)
let _testKeyQ = null;
let _testKeyW = null;

// 오답 리뷰 모드 상태 변수
let incorrectIndices = [];
let incorrectGlobalIndices = [];
let currentIncorrectIndex = 0;
let originalNextFunction = null;
let originalPrevFunction = null;
...
function handleKeyboardEvent(event) {
  ...
  if (event.key === 'q' || event.key === 'Q') { _testKeyQ = Date.now(); }
  if (event.key === 'w' || event.key === 'W') { _testKeyW = Date.now(); }
  if ((event.key === 'q' || event.key === 'Q' || event.key === 'w' || event.key === 'W') &&
      _testKeyQ && _testKeyW && Math.abs(_testKeyQ - _testKeyW) < 300) {
    _testKeyQ = null;
    _testKeyW = null;
...
async function getLastQuestionNumberForMockExam(year, hour) {
  try {
    ...
    // Firebase에서 세션의 attempts 가져오기 (static import 사용)
    // ✅ Firebase 초기화 확인
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  // 세션 초기화 시도
  try {
    // ✅ Firebase 초기화 먼저 수행 (static import 사용)
    await ensureFirebase();
  ...
  Promise.all(
    ...
    .then(async data => {
      ...
      if (interrupted) {
        ...
        if (choice === 'resume') {
          ...
          if (interrupted.perQuestionChecked) {
            perQuestionChecked = interrupted.perQuestionChecked;
          }
      ...
      // 선택지 버튼 이벤트 위임 (onclick 대체)
      document.getElementById('option-buttons')?.addEventListener('click', (e) => {
      ...
      // 결과 화면 이벤트 위임 (innerHTML onclick 대체)
      document.getElementById('results-summary')?.addEventListener('click', (e) => {
        const btn = e.target.closest('[data-action]');
        if (!btn) return;
        switch (btn.dataset.action) {
          case 'review': reviewQuiz(); break;
          case 'go-home': goHomeAfterSave(); break;
          case 'review-subject': reviewSubjectIncorrect(btn.dataset.subject); break;
        }
      });
...
function changeSubject(subject) {
  ...
  if (subject === 'all') {
    ...
    if (wasInReviewMode) {
      ...
      const currentIncorrectGlobalIndex = incorrectGlobalIndices ?
        incorrectGlobalIndices[currentIncorrectIndex] : null;
      ...
      // 전체 오답 인덱스로 업데이트
      incorrectIndices = allIncorrectIndices;
      incorrectGlobalIndices = allIncorrectGlobalIndices;
    ...
    if (wasInReviewMode) {
      // 현재 전체 오답 인덱스 저장
      const originalIncorrectIndices = incorrectIndices || [];
      const originalGlobalIndices = incorrectGlobalIndices || [];
      ...
      if (subjectIncorrectIndices.length > 0) {
        ...
        incorrectIndices = subjectIncorrectIndices;
        incorrectGlobalIndices = subjectIncorrectGlobalIndices;
        currentIncorrectIndex = 0;
  ...
  if (wasInReviewMode) {
    // 리뷰 모드 UI가 아직 없으면 추가
    if (!document.querySelector('.review-mode-message')) {
      const incorrectCount = incorrectIndices ? incorrectIndices.length : 0;
...
function selectOption(optionIndex) {
  ...
  try {
    ...
    const saveData = {
      answers: userAnswers,
      perQuestionChecked: perQuestionChecked || new Array(questions.length).fill(false),
...
function checkAnswer() {
  ...
  if (!perQuestionChecked) {
    perQuestionChecked = new Array(questions.length).fill(false);
  }
  perQuestionChecked[currentQuestion.globalIndex] = true;
  ...
  try {
    ...
    const saveData = {
      answers: userAnswers,
      perQuestionChecked: perQuestionChecked,
      lastQuestionIndex: currentQuestionIndex,
      timestamp: Date.now()
    };
    localStorage.setItem(localStorageKey, JSON.stringify(saveData));
...
function updateQuestionIndicators() {
  ...
  indicators.forEach((indicator, i) => {
    ...
    if (isAnswered && !reviewMode && perQuestionChecked && perQuestionChecked[globalIndex]) {
...
async function saveMockExamResults() {
  ...
  // 방법 2: modular Firebase auth에서 직접 가져오기
  ...
  for (let globalIndex = 0; globalIndex < questions.length; globalIndex++) {
    ...
    const viewedExplanation = __mockViewedExplanation[globalIndex] === true
      || (perQuestionChecked && perQuestionChecked[globalIndex] === true);
...
function updateIncorrectCounter(totalIncorrect) {
  ...
  if (infoElement) {
    const total = totalIncorrect || (incorrectIndices ? incorrectIndices.length : 0);
    const current = currentIncorrectIndex + 1;

    // 현재 과목 표시 추가
    if (currentSubject !== 'all') {
      infoElement.textContent = `${currentSubject}: 틀린 ${total}문제 중 ${current}번째 (전체 오답: ${incorrectGlobalIndices ? incorrectGlobalIndices.length : total}개)`;
    } else {
...
function reviewQuiz() {
  ...
  if (!originalNextFunction) {
    originalNextFunction = goToNextQuestion;
    originalPrevFunction = goToPreviousQuestion;
  }
...
function showResults() {
  ...
  // 오답 문제 수집
  const localIncorrectIndices = [];
  const localIncorrectGlobalIndices = [];
  ...
  questions.forEach((q, i) => {
    ...
    if (!isCorrect && userAnswer !== null) {
      localIncorrectIndices.push(i);
      localIncorrectGlobalIndices.push(q.globalIndex);
    }
  });
  ...
  // 모든 문제 맞았는지 확인
  if (localIncorrectIndices.length === 0) {
    // 모든 문제 맞은 경우
    ...
    // 오답 인덱스를 모듈 변수에 저장
    incorrectIndices = localIncorrectIndices;
    incorrectGlobalIndices = localIncorrectGlobalIndices;
    currentIncorrectIndex = 0;
  }
...
function goToPreviousIncorrect() {
  if (currentIncorrectIndex > 0) {
    ...
    currentQuestionIndex = incorrectIndices[currentIncorrectIndex];

    // 버튼 상태 업데이트
    document.getElementById('prev-button').disabled = currentIncorrectIndex === 0;
...
function goToNextIncorrect() {
  if (currentIncorrectIndex < incorrectIndices.length - 1) {
    currentIncorrectIndex++;
    currentQuestionIndex = incorrectIndices[currentIncorrectIndex];

    // 버튼 상태 업데이트
    document.getElementById('prev-button').disabled = false;
    document.getElementById('next-button').disabled = currentIncorrectIndex >= incorrectIndices.length - 1;
...
function exitReviewMode() {
  ...
  prevButton.addEventListener('click', originalPrevFunction || goToPreviousQuestion);
  nextButton.addEventListener('click', originalNextFunction || goToNextQuestion);
